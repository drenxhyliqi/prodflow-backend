...
class AiController extends Controller
{
    ...
    private function buildSystemPrompt(string $message, int $companyId): string
    {
        ...
        if ($has('sales')) {
            ...
            $topClients = \App\Models\SalesModel::where('company_id', $companyId)->whereMonth('date', $thisMonth)->whereNotNull('client')->selectRaw('client, SUM(qty * price) as total, COUNT(*) as orders')->groupBy('client')->orderByDesc('total')->limit(5)->get();
